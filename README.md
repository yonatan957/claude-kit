...
pytest                              # everything except the live suite
pytest tests/live --run-live        # opt-in: real network
python tools/check_file_length.py src tests tools
lint-imports
```

Two checks are gates, not suggestions: no Python file exceeds 100 non-blank, non-comment
lines, and the import direction `cli → application → ports → domain` holds with
`application` never importing an adapter. Both fail CI.

See [`specs/001-package-management-cli/`](specs/001-package-management-cli/) for the
specification, plan, and architecture, and
[`.specify/memory/constitution.md`](.specify/memory/constitution.md) for the principles
they answer to.
